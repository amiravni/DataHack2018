...
diffFalg=False  # use image diff.
showLabel=False # show labels
KeyFrameUpdate=50 # [frames]
writeLabelFlag=True # write new labels
ShowResults=False # plot scatter

if writeLabelFlag:
    print('WARNING!!! Writing new Labels')
...
def clustMake(centroid,idx,X):
    if idx>min_idx and len(centroid)>0:
...
def pcTopView(x,y,z,d,label,img_prev,idx,diffFalg=False):
    ...
    idxX = np.round((maxDist_x/2+x)*pix2m_x).astype('uint32')   
    
   
    
    for iii in range(0,len(d)):
        if idxY[iii]>EL_NUM or idxX[iii]>AZ_NUM:
            print('WARNING! X_Y indexes out of bounce ',str(idxY[iii])+' / '+str(idxX[iii]))
        try:
            
            if z[iii]>0.6 and z[iii]<1.9:
                if showLabel:
                    if label[iii]==1:
    ...
    if len(X[0]) >=30 and tmp[0] !=[] and tmp[1] !=[]:
        X=X.T
...
if __name__ == '__main__':
    ...
    for idx, frame in enumerate(data_utils.enumerate_frames(video_dir)):
        ...
        if idx%KeyFrameUpdate==0 or idx==1000:
            ...
            imgSum2Show[np.where(imgSum2Show > 0.3)] = 0
            plt.imshow(imgSum2Show)
            #plt.show()
            time.sleep(0.01)
            
            for idxTmp,prev_img_tmp in enumerate(prev_img_list):
                label_pred = np.zeros(cloud_len_list[idxTmp]) 
                idxXY_tmp = idxXY_list[idxTmp] 

                for iii in range(0,cloud_len_list[idxTmp]):
                    if idxXY_tmp[1][iii]>EL_NUM or idxXY_tmp[0][iii]>AZ_NUM:
                        print('WARNING! X_Y indexes out of bounce '+str(iii))
                        continue
                ...
                if writeLabelFlag:
                    print('saving '+str(video_dir+"\\"+fn_list[idxTmp]+"_labels.csv"))
                    np.savetxt(video_dir+"\\"+fn_list[idxTmp]+"_labels.csv", label_pred,fmt='%i', delimiter=",")
            if idx>min_idx:
                imgSum = np.zeros((EL_NUM,AZ_NUM))
                prev_img_list=[]
                idxXY_list = []
        ...
        # norm1_y=-y/x
        # norm_y=(norm1_y-min(norm1_y))/(max(norm1_y)-min(norm1_y))
        # norm1_z=-z/x
        # # tmp=np.linalg.norm(norm1_z, axis=0)
        # # norm2 = normalize(norm1_z[:,np.newaxis], axis=0).ravel()
        # norm_z=(norm1_z-min(norm1_z))/(max(norm1_z)-min(norm1_z))
        ...
        fn_list.append(fn)
        cloud_len_list.append(pc.shape[0])
