...
def read_all_data(folder, index, mounting_vector=(0.6, 0., 1.3)):
    ...
    :return:
    """
    pc,fn = read_data(folder, index, 'pointcloud')
    pc[:, :3] += np.array(mounting_vector)
    try:
        ego_motion,fn = read_data(folder, index, 'egomotion')
    except FileNotFoundError:
        ego_motion = None

    try:
        labels,fn = read_data(folder, index, 'labels')
    except FileNotFoundError:
        labels = None
        
    try:
        label_pred,fn = read_data(folder, index, 'label_pred')
    except FileNotFoundError:
        label_pred = None

    return pc, ego_motion, labels,label_pred,fn
...
def frame_to_filename(folder, index, data_type):
    valid_input = {'pointcloud', 'egomotion', 'labels','label_pred'}
